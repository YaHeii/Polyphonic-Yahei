...
CREATE TABLE t_user (
    ...
    register_type smallint NOT NULL DEFAULT 0,
...
COMMENT ON COLUMN t_user.status IS '状态: 0正常 1禁用 2 删除';
...
CREATE TABLE t_role (
    ...
    CONSTRAINT ck_role_key_enum CHECK (role_key IN ('root', 'super_admin', 'user'))
);
...
COMMENT ON COLUMN t_article.article_type IS '文章类型 0原创 1转载 2翻译';
...
COMMENT ON COLUMN t_article.status IS '状态值 0公开 1私密 2草稿';
...
CREATE TABLE t_comment (
    ...
    reply_user_id varchar(64) NOT NULL DEFAULT '',
    is_delete boolean NOT NULL DEFAULT false,
...
COMMENT ON COLUMN t_comment.parent_id IS '顶层评论id';
COMMENT ON COLUMN t_comment.reply_id IS '直接回复的评论id';
COMMENT ON COLUMN t_comment.reply_user_id IS '被回复用户id';
COMMENT ON COLUMN t_comment.is_delete IS '是否删除 0否 1是';
COMMENT ON COLUMN t_comment.comment_content IS '评论内容';
COMMENT ON COLUMN t_comment.type IS '评论类型 0.文章 1.说说';
COMMENT ON COLUMN t_comment.status IS '状态 0.正常 1.已删除';
...
COMMENT ON COLUMN t_message.is_top IS '是否置顶 0否 1是';
COMMENT ON COLUMN t_message.is_delete IS '是否删除 0否 1是';
COMMENT ON COLUMN t_message.status IS '状态 0.正常 1.已删除';
...
CREATE INDEX idx_comment_topic_type ON t_comment (topic_id, type);
CREATE INDEX idx_comment_topic_parent ON t_comment (topic_id, parent_id);
CREATE INDEX idx_comment_reply_id ON t_comment (reply_id);
